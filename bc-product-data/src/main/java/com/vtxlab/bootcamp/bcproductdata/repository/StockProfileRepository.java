...
public interface StockProfileRepository extends JpaRepository<ProfileEntity, Long> {
  ...
  @Query(value = "SELECT p FROM ProfileEntity p WHERE p.quoteStockCode = :symbol ORDER BY p.quoteDate DESC LIMIT 1")
  ProfileEntity getMostRecentProfileEntityBySymbol(@Param("symbol") String symbol);
