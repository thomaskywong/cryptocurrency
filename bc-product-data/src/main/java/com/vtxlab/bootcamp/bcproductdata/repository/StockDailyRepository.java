...
public interface StockDailyRepository extends JpaRepository<StockDailyEntity, Long> {
  ...
  @Query(value = "SELECT se FROM StockDailyEntity se WHERE se.stockIdEntity2 = :stockIdEntity AND se.tradeDate = :date")
  StockDailyEntity getDailyEntityByDateAndSymbol(@Param("stockIdEntity") StockIdEntity stockIdEntity, @Param("date") LocalDate date);
